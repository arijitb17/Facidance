...
# ---------------------------------------------------------------------------
# Course students (detail view) — parallel photo fetching
# ---------------------------------------------------------------------------
...
async def get_course_students(user_id: str, course_id: str) -> dict:
    ...
    Photo data is fetched in parallel (asyncio.gather) instead of
    one-by-one, so the endpoint doesn't hang for large classes.
    """
    t0 = time.perf_counter()
    try:
        ...
        # 4. Fetch ALL photo data in parallel
...
async def remove_student_from_course(user_id: str, course_id: str, data) -> dict:
    t0 = time.perf_counter()
    try:
        teacher = await prisma.teacher.find_unique(where={"userId": user_id})
        if not teacher:
